...
WebScraper.prototype.start = function(host, paths, script, options) {
	if (!this.running) {
	    this.running = true;
		...
		if (this.unvisited.length > 0) {
			...
			request(self.options, function (error, response, body) {
				...
				try {
					...
					vm.runInNewContext(self.script, self.sandbox, {displayErrors: true});
				} catch (e) {
...
Scraper.prototype.run = function (html, script, modules, cheerioInstance) {
	this.html = html || '';
	this.script = script || '';
	this.emit('run');

	// Allow to use a given cheerio instance
	if (!cheerioInstance) {
	  cheerioInstance = cheerio.load(this.html);
	}

	this.sandbox.$ = cheerioInstance;
	...
	if (modules) {
		Object.keys(modules).forEach(function(key) {
			...
			that.sandbox[key] = moduleInstance;
		});
	}

	try {
		const script = new vm.Script(this.script.toString());
		script.runInNewContext(this.sandbox, {displayErrors: true});
